...
| **G4** | HIGH→MODERATE | Computational tractability confirmed — tree grows exponentially with base ~2.3 (corrected from ~4.7 per G7 resolution) | Depth-7 (88 nodes, not 588) computable in milliseconds. Depth-20 (~4×10⁶ nodes) tractable on commodity hardware. The framework is far more computable than previously claimed. |
...
| **G7** | HIGH→RESOLVED | ~~Tree generation [1,2,2,3,7,16,38,88] ≠ documented [1,2,2,3,7,28,125,588].~~ **RESOLVED 2026-07-21:** The documented numbers in `f-contractiveness-analysis.md` §5 were incorrect. That document claimed they were "verified from the executable" but the actual code `_self_descriptive_system.py` (unchanged since initial commit `fe48edc`) produces [16,38,88] with asymptotic ratio ~2.3. `f-contractiveness-analysis.md` §5 corrected to match actual code output. All downstream documents updated. | `f-contractiveness-analysis.md` §5 corrected. Audit: 92 raw candidates → 49 kept from depth-4 nodes (46.7% collision rate). Double-Enclosure + Cancellation + Condensation cause the reduction. |
