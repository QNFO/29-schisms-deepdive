...
| **17** | **Task 1.4a: C* Extension — Reach T* at Depth ≥ 2** | ✅ **EXECUTED** (2026-07-21) — `task-1.4a-cstar-extension.md`. 5 unique depth-≥2 T* found (#[], []→#, #[]#, [#[]], [[]#]). NO T* subtree matches executable [1,2,2,3,7] prefix. Asymptotic ratio ~4.7 claimed but UNVERIFIED — see G7 (tree generation discrepancy found in Task 1.4b). |
| 18 | Task 1.4b: Deep Subtree Comparison + G7 Tree Audit | ✅ **EXECUTED** (2026-07-21) — `task-1.4b-subtree-comparison.md`. Tree built to depth 7 but tree-generation DISCREPANCY found: actual growth [1,2,2,3,7,16,38,88] vs documented [1,2,2,3,7,28,125,588]. 46.7% of candidate children eliminated by reduction rule collisions. Documented ~4.7 asymptotic ratio UNVERIFIED. New gap G7 filed — `f-contractiveness-analysis.md` §5 numbers suspect. **Highest priority: resolve G7 before continuing subtree analysis.** |
...
**#1 — Resolve G7 (Tree Generation Discrepancy, HIGH):** The tree generated by `_self_descriptive_system.py` produces [1,2,2,3,7,16,38,88] not documented [1,2,2,3,7,28,125,588]. 46.7% of candidate children eliminated by reduction rule collisions. `f-contractiveness-analysis.md` §5 numbers are suspect. Must resolve: (a) are the documented numbers from a different generation method? (b) should the code be fixed or the docs corrected? This blocks all subtree analysis.

**#2 — Task 2.1-2.4 (External Validation, 1-3 months calendar, unblocked):** Send validator emails once Cloudflare Email Service domain is onboarded.
