...
| 7a | Task 2.2: Prepare taxonomy validation package | ✅ **DONE** (2026-07-20) — `taxonomy-validation-package.md`: QNFO-neutral standalone review document, 29 schisms with rich descriptions, 5-layer structure, classification form (P/Ph/B × U/C/R/I × H/M/L), missing-schisms field. Self-contained — validators need no prior framework knowledge. |
| 8 | Validator outreach: send emails + Cloudflare Email Worker | ⬜ Pending — 5 personalized emails drafted in `validator-outreach-emails.md`. Infrastructure: Outlook COM script ready, Cloudflare Email Service Worker template ready. Requires: (a) final publication, (b) domain onboarding (SPF/DKIM), (c) user approval. |
| 8a | OSF pre-register trapped-ion protocol (Task 3.1) | ⬜ Pending — protocol document complete, ready to submit, requires explicit user approval per OSF Bona Fide Registration Requirements |
