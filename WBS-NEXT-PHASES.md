...
**Closed (this session, 2026-07-21):** Formal paper published and expanded to 20 pages (v2.1→v2.2) with full schism descriptions and resolution matrix appendix; Zenodo canonical version chain repaired (v1.2→v2.2, orphan v2.0/v2.1 documented); Task 1.3 (ancestor-monotone map characterization, 6 theorems) and Task 1.3a (real depth-4/5 numerical verification, 48/48 fail) completed with actual executed evidence; **Task 1.3b ADOPTED — trajectory-local Bootstrap Conjecture reframe** (new document `trajectory-local-bootstrap-conjecture.md`, `calibration-map-c-definition.md` → v2.1, `c-contractiveness-proof.md` → v2.1); **Task 1.3c PARTIAL PROOF DELIVERED** — T6.1 proven for constrained class (ALM + parent-map fallback), gap to general proof is Lemma G (characterized but unproven); **Task 1.4 PRELIMINARY CHARACTERIZATION COMPLETE** — `valuation-structure-characterization.md`: branching factors are tree-intrinsic (not map-dependent); all 36 valid T* at depth 1 (18 terminal `#`, 18 container `[]`); asymptotic ratio ~4.7 confirmed universal; early-level [1,2,2,3,7] match requires C* extension (Task 1.4a); 2 spin-off research handoffs drafted; a genuine mathematical error in `f-contractiveness-analysis.md` was found and corrected (see #14).

**Genuinely open (the real remaining work):**
- **#6c (Lemma G) — the remaining mathematical gap.** Full T6.1 awaits proof of Lemma G (whether F must be ancestor-monotone for deep nodes under global non-expansiveness). Constrained-class proof is complete. Trajectory-local reframe (#6b, adopted) sidesteps this gap.
...
- **#14 — the f-contractiveness-analysis.md erratum** — already applied (Corollary 1 corrected).
