...
| 18 | Task 1.4b: Deep Subtree Comparison + G7 Tree Audit | ✅ **EXECUTED** (2026-07-21) — `task-1.4b-subtree-comparison.md`. Tree built to depth 7, tree-generation DISCREPANCY found and **RESOLVED**. Actual growth [1,2,2,3,7,16,38,88] with asymptotic ~2.3. `f-contractiveness-analysis.md` §5 corrected to match code output. G7 closed. **Highest priority now: Task 2.1-2.4 (External Validation).** |
...
**#1 — Task 2.1-2.4 (External Validation, 1-3 months calendar, unblocked):** Send validator emails once Cloudflare Email Service domain is onboarded. G7 is now RESOLVED (docs corrected to match code).

**#2 — Task 4.1-4.4 (CMB Log-Periodic Search, 1-3 months compute, unblocked):** Pipeline documented, data is public (Planck, ACT, SPT). No lab access needed.
