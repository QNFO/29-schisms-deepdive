...
| 6b | Task 1.3b: Reframe Bootstrap Conjecture (Trajectory-Local formulation) | ⬜ **PROPOSED, NOT ADOPTED** — `deeper-math-bootstrap-obstruction.md` §5 proposes relaxing global non-expansiveness to trajectory-local non-expansiveness. Cost: Banach's theorem no longer applies; T* uniqueness not guaranteed. **Decision needed:** adopt this reframe, or continue searching for a genuine global counterexample at depth 6+. If adopted: rewrite `calibration-map-c-definition.md` and `c-contractiveness-proof.md` to state the relaxed conjecture as primary. |
| 6c | Task 1.3 rigor gap: analytic proof of T6.1 | ⬜ **OPEN — the actual unresolved mathematical core of the project.** Current evidence is numerical (finite depth ≤5) plus a heuristic failure-mode taxonomy (`deeper-math-bootstrap-obstruction.md` §3: Parent-Collapse / Branch-Convergence / Trajectory-Oscillation). A full proof requires showing Failure Mode A (parent-collapse) is structurally unavoidable at ANY depth for ANY non-expansive map with non-trivial T* — not just empirically absent up to depth 5. This is genuinely hard, unsolved, and the correct next research priority for anyone continuing Phase 1. |
...
| 14 | Correct known-false claim in `f-contractiveness-analysis.md` | ⬜ **NEW, discovered 2026-07-21** — that document's Corollary 1 states "the parent function is ALWAYS contractive." `ancestor-monotone-map-characterization.md` §2 proves this is **false** with an explicit counterexample: DIST doubles for ancestor/descendant pairs under the parent map (e.g. `DIST(parent(A), parent(B)) = 2 × DIST(A, B)` when B is a child of A). This false claim is committed, R2-synced, and was previously cited as established fact in earlier proof documents. Needs an erratum edit in `f-contractiveness-analysis.md` itself, not just a correction noted elsewhere. |
...
### Summary: What Is Genuinely Open vs. Closed

**Closed (this session, 2026-07-21):** Formal paper published and expanded to 20 pages (v2.1→v2.2) with full schism descriptions and resolution matrix appendix; Zenodo canonical version chain repaired (v1.2→v2.2, orphan v2.0/v2.1 documented); Task 1.3 (ancestor-monotone map characterization, 6 theorems) and Task 1.3a (real depth-4/5 numerical verification, 48/48 fail) completed with actual executed evidence; 2 spin-off research handoffs drafted; a genuine mathematical error in `f-contractiveness-analysis.md` was found (not yet corrected — see #14).

**Genuinely open (the real remaining work):**
- **#6b/#6c — the mathematical core.** Either prove T6.1 analytically or decide to adopt the trajectory-local reframe. This is the load-bearing unresolved question of the entire project.
- **#8 — send the validator emails** (drafting done, needs domain setup + user go-ahead).
- **#9/#10 — OSF pre-registration** (blocked on user approval, not effort).
- **#11 — PBO/D1 deployment** (blocked on infrastructure, status stale).
- **#12/#13 — the two spin-off projects** exist only as handoff documents; no research has actually been performed on either topic yet. They require fresh sessions in new repos.
- **#14 — the f-contractiveness-analysis.md erratum** is a small, well-defined, immediately actionable fix that has not yet been made.
